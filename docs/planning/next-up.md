...
- **Merged-config MCP projection — Codex shipped v0.9.0; other targets pre-staged.** Design in [`mcp-merge-design.md`](mcp-merge-design.md); user-facing walkthrough in [`docs/walkthrough.md`](../walkthrough.md#sharing-mcp-with-codex-csaw-mcp-sync). `csaw mcp sync codex --from <source>` merges `[mcp_servers.*]` entries into `.codex/config.toml` via a bounded section (user content byte-for-byte preserved; rollback drops the section, drift detected via SHA). Open targets (OpenCode `opencode.json`, Copilot CLI `~/.copilot/mcp-config.json`, VS Code workspace `settings.json`) use the same architecture but aren't wired — add when a real user reports friction on those specific tools.
- **Vendor feature (`csaw vendor add/sync/list/audit/promote/remove`) — shipped v0.10.0.** Design in [`vendors-design.md`](vendors-design.md); walkthrough in [`docs/walkthrough.md`](../walkthrough.md#vendoring-external-catalogs-safely-csaw-vendor). Lets registry maintainers consume external catalogs (skills.sh, APM packages, awesome-copilot, any git repo) into a hash-locked `vendor/` area with three drift detectors (vendor-local, upstream, promotion) and explicit promote-to-real-kind step. Vendored content is excluded from source enumeration — it never projects until promoted. Strategically positions csaw as the safe consumer of any ecosystem (APM, skills.sh, etc.) rather than a competitor.
